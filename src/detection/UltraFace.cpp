...
void UltraFace::infer()
{
    ...
    std::cout << "The queue reference is valid\n";
    ...
    while (!frame_queue_ref.empty()) {
        ...
        if (!frame.empty()) {
            std::cout << "Frame obtained from queue\n";
            ...
            ultraface_queue.push(std::move(frame));
        }
    }
    ...
    int UltraFace::detect(ncnn::Mat & img, std::vector<FaceInfo> & face_list)
    {
        ...
        generateBBox(bbox_collection, scores, boxes, score_threshold,
                     num_anchors);
        ...
        for (int i = 0; i < num_anchors; i++) {
            if (scores.channel(0)[i * 2 + 1] > score_threshold) {
                ...
                float x_center =
                    ...
                    priors[i][0];
                float y_center = boxes.channel(0)[i * 4 + 1] * center_variance *
                                     priors[i][3] +
                                 priors[i][1];
                float w = exp(boxes.channel(0)[i * 4 + 2] * size_variance) *
                          priors[i][2];
                float h = exp(boxes.channel(0)[i * 4 + 3] * size_variance) *
                          priors[i][3];
    ...
    void UltraFace::nms(std::vector<FaceInfo> & input,
